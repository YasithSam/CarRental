...
include "../../db/connection.php";
  ...
  && isset($_POST['name']) && isset($_POST['city'])&& isset($_POST['phone'])&& isset($_POST['confirm_password'])) {
    function validate($data){
     ...
     if($pass !== $re_pass){
        header("Location: ../driver-form.php?error=The confirmation password  does not match");
	    exit();
	}
     $pass = md5($pass);
		...
		if (mysqli_num_rows($result) > 0 || mysqli_num_rows($result2)>0) {
			header("Location: ../driver-form.php?error=The username is taken try another&$user_data");
	        exit();
		}else {
           ...
           if ($result2) {
                ...
                exit();
	           	...
	           	header("Location: ./driver-form.php?error=unknown error occurred");
		        exit();
           }
		}
        ...
        header("Location: ./driver-form.php");
        exit();
    }
